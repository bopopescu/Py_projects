...
def check_package():
	...
	if data :
		...
		if package_flag == '1':
			if PKG_Install_flag == True: 
				...
				if Multiple_recipients == True :
					...
					#session.sendmail(sender, recipients, msg.as_string())##################################### Enable ##################################
				else :
					print "\n Sending email from:" , sender, "to: " , recipient1
					#session.sendmail(sender, recipient1, msg.as_string())##################################### Enable ##################################
				session.quit()
				...
				print "\n-----INFO: Package type is : ", package_type
				print "\n-----INFO: Skipping package installation part since PKG_Install_flag is:  " ,PKG_Install_flag
				# Email notification for starting package installation, sent to station owner and jian :
				# Send the message via local SMTP server.
				session = smtplib.SMTP(server)
				# Create message container - the correct MIME type is multipart/alternative.
		...
		if PKG_Install_flag == True: 
			...
			#session.sendmail(sender, recipient1, msg_2.as_string())##################################### Enable ##################################
			session.quit()
			
			print "\n-----INFO: Calling QA Script : Run_QA.py"
			package_name ="IQfact+_QCA_6174_3.2.1.RC4" ##################################### Remove ##################################
			...
			#db.commit() ##################################### Enable ###################################commit the Updates made to database
			...
			# and message to send - here it is sent as one string.
			print "\n Sending email from:" , sender, "to: " , recipient1
			#session.sendmail(sender, recipient1, msg_3.as_string())##################################### Enable ##################################
			session.quit()
