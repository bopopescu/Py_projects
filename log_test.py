...
def cirarr1(nums): # bool
    ...
    for i, v in enumerate(nums):
        ...
        pos_dict = set()
        if nums[i] > 0:
            while nums[i] > 0 and cnt <= len(nums):
                visit[i] = 1
                if nums[i] in pos_dict and cnt > 1:
                    return True
            ...
            while nums[i] < 0 and cnt <= len(nums):
                visit[i] = 1
                if nums[i] in neg_dict and cnt > 1:
                    return True
                if nums[i] in neg_dict:
                    return False
                neg_dict.add(nums[i])
                cnt += 1
                i = (i-nums[i]) if i>= nums[i] else i+len(nums)-nums[i]
    return False
