...
if  Measurement_flag==1:
	...
	evmsheet.write(0,7,"EVM_Value dB")
	evmsheet.write(0,8,"Upper Limit")
	evmsheet.write(0,9,"Lower Limit")
	evmsheet.write(0,10,"Target power dBm")
	evmsheet.write(0,11,"Power Average dBm")
	evmsheet.write(0,12,"Upper Limit")
	evmsheet.write(0,13,"Lower Limit")
	evmsheet.write(0,14,"Delta_Power_dB")
	...
	powersheet.write(0,7,"Power Value dBm")
	powersheet.write(0,10,"Target Power dBm")
	powersheet.write(0,11,"Delta Power dBm")
	powersheet.write(0,8,"Upper Limit")
	powersheet.write(0,9,"Lower Limit")
...
evm_matrix=np.zeros((1000,1000))
power_matrix=np.zeros((1000,1000))
evmpower_matrix=np.zeros((1000,1000))
...
for line in f:
	...
	if collect_data==1:
		...
		if 'EVM_AVG_DB' in line and ref_evm_number==current_test_num: #collect output result for evm values
			...
			evm_matrix[runID-1,current_test_num-1]=evm_values			
			...
			if Measurement_flag==1:
				if runID==firstID:
					...
					evmsheet.write(evmcounter,8,(up_limit))
					evmsheet.write(evmcounter,9,(lo_limit))
					evmsheet.write(evmcounter,7,evm_values)
					
				else:
						
					for x in range(1,runID):
						if evm_matrix[x-1][current_test_num-1]==0:
							negative+=1
							
						evm_sum+=(evm_matrix[x-1][current_test_num-1])
		...
		if 'POWER_AVG_DBM' in line and ref_evm_number==current_test_num: #collect output result for power values
			...
			if (((evmpower_values-Target_power)> 1.5 or (evmpower_values-Target_power)< -1.5)) and evmpowerenable==1 and Power_QA_report==1:
								...
								QAreportsheet.write(QAmessagecounter,5,packet_format)
								QAmessagecounter+=1
			evmpower_matrix[runID-1,current_test_num-1]=evmpower_values
			evmpower_sum=0
			if Measurement_flag==1:
				if runID==firstID:
								
					evmsheet.write(evmcounter,12,up_limit)
					evmsheet.write(evmcounter,13,lo_limit)
					evmsheet.write(evmcounter,11,evmpower_values)
					evmsheet.write(evmcounter,14,evmpower_values-temp_power)
				else:
					for x in range(1,runID):
						if evmpower_matrix[x-1][current_test_num-1]==0:
								negative+=1
								
						evmpower_sum+=(evmpower_matrix[x-1][current_test_num-1])
		...
		if 'POWER_AVERAGE_DBM' in line and ref_power_number==current_test_num: #collect output result for power values
			...
			if Measurement_flag==1:
				if runID==firstID:
					...
					for x in range(1,runID):
						if power_matrix[x-1][current_test_num-1]==0:
							negative+=1
							
						power_sum+=(power_matrix[x-1][current_test_num-1])
		...
		if 'TX_POWER_DBM ' in line and ref_evm_number==current_test_num: #collect Target Power values for EVM  
			...
			if runID==firstID:
				...
				Target_power=float(col[2])
				if Measurement_flag==1:	
					evmsheet.write(evmcounter,10,Target_power)
				temp_power=Target_power
